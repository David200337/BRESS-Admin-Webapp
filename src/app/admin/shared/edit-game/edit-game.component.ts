...
export class EditGameComponent implements OnInit {
      ...
      if (bothSet || bothNull) {
        if (bothSet) {
          if (set[0] < 11 && set[1] < 11) {
            this.errorMessage = `Er is niet minimaal 11 punten in set ${
              index + 1
            } voor de winnaar`;
            return false;
          } else if (set[0] == set[1]) {
            ...
            if (Math.abs(set[0] - set[1]) != 2) {
              this.errorMessage = `Er is precies een verschil van 2 nodig in set ${
                index + 1
              }`;
              return false;
            }
            ...
            if (Math.abs(set[0] - set[1]) < 2) {
              this.errorMessage = `Er is minimaal een verschil van 2 nodig in set ${
                index + 1
              }`;
              return false;
            }
  ...
  onSubmit() {
    if (this.validateScores()) {
      ...
      this.editGame
        .enterScore(this.playerScores, this.isEdit)
        .subscribe((res) => {
          console.log(res);

          this.sumbitEvent.emit({
            id: this.editGame.gameId,
            score: this.playerScores,
          });
          this.playerScores = [
